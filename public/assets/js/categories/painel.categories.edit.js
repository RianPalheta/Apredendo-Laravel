$(function() {
    ...
    $('#categories-edit').submit(function(e){
        e.preventDefault();
        if(btn_click !== false) return null;
        let form = document.getElementById('categories-edit');
        ...
        request.addEventListener('load', function(e) {
            ...
            if(request.status === 200) {
                ...
                if(r.success === true) {
                    toastr["success"]("Categoria editada com sucesso!", "Sucesso");
                } else {
...
function organizeCategory(array = [], level) {
    ...
    let option = array.map(i => {
        let option = `
        <option value="${i.id}" ${i.id == opt_sub ? 'selected="selected"' : ''}>
